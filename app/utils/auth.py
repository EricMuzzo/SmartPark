...
from ..models.generic import TokenData
...
def create_access_token(username: str, expires_delta: timedelta | None = None):
    ...
    if expires_delta is not None:
        ...
        expire = datetime.now(timezone.utc) + timedelta(minutes=int(ACCESS_TOKEN_EXPIRES_MINUTES))
    to_encode.update({"exp": expire})
    encoded_jwt = jwt.encode(to_encode, SECRET_KEY, algorithm=ALGORITHM)
    return encoded_jwt


async def get_current_user(token: Annotated[str, Depends(oauth2_scheme)]) -> UserCreate:
    ...
    user = await get_user(username=token_data.user_name)
    if user is None:
        raise credentials_exception
    return user


async def get_current_active_user(current_user: Annotated[UserCreate, Depends(get_current_user)]) -> UserCreate:
    return current_user